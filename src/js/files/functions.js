...
export function spollers() {
	...
	if (spollersArray.length > 0) {
		...
		const spollersRegular = Array.from(spollersArray).filter(function (item, index, self) {
			return console.log(!item.dataset.spollers.split(",")[0]	)
		});
...
export function showMore() {
	...
	function getHeight(showMoreBlock, showMoreContent) {
		let hiddenHeight = 0;
		const showMoreType = showMoreBlock.dataset.showmore ? showMoreBlock.dataset.showmore : 'items';
	...
	function showMoreActions(e) {
		...
		console.log(targetEvent.closest('[data-showmore-button'))
		...
		if (targetType === 'click') {
			if (targetEvent.closest('[data-showmore-button]')) {
				...
				if (!showMoreContent.classList.contains('_slide')) {
					...
					showMoreButton.classList.toggle('companies__showmore_active')
				}
			}
